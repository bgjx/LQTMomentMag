...
def plot_spectral_fitting(
    ...
    for station_idx, (stream, p_time, s_time, station) in  enumerate(zip(streams, p_arr_times, s_arr_times, stations)):
        ...
        for comp, label in zip(["L", "Q", "T"], ["P", "SV", "Sh"]):
            ...
            ax.axvline(p_time - CONFIG.PADDING_BEFORE_ARRIVAL - trace.stats.starttime, color='g', linestyle='--')
            ...
            ax.axvline(p_time - CONFIG.NOISE_DURATION - trace.stats.starttime, color='gray', linestyle='--')
            ax.axvline(p_time - CONFIG.NOISE_PADDING - trace.stats.starttime, color='gray', linestyle='--')
